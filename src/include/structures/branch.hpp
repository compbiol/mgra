...
struct Branch {
    ...
    structure::phyl_tree::Node<mcolor_t> convert_to_node() const {
        ...
        node_t node(color_center, std::shared_ptr<node_t>(new node_t(left)), std::shared_ptr<node_t>(new node_t(right)));
        return node;
    }
    ...
    void fill_node(std::shared_ptr<structure::phyl_tree::Node<mcolor_t>> const & node) const {
        using node_t = typename structure::phyl_tree::Node<mcolor_t>;
        node->set_left_child(std::make_shared<node_t>(mcolor_t(node->get_data(), left, mcolor_t::Intersection)));
        node->set_right_child(std::make_shared<node_t>(mcolor_t(node->get_data(), right, mcolor_t::Intersection)));
    }
    ...
    */
    bool node_left_includes(std::shared_ptr<structure::phyl_tree::Node<mcolor_t>> const &node) const {
        return node->get_left_child()->get_data().includes(left);
    }

    bool node_right_includes(std::shared_ptr<structure::phyl_tree::Node<mcolor_t>> const &node) const {
        return node->get_right_child()->get_data().includes(left);
    }
