...
namespace structure {
  ...
  struct Node {
    ...
    node_unique_ptr const& get_left_child() const {
      return left_child;
    }

    node_unique_ptr const& get_right_child() const {
      return right_child;
    }
  ...
  private:
    ...
    Node* const parent;
    node_unique_ptr left_child;
    node_unique_ptr right_child;
  };
    ...
    if (new_tree[0] == '(') {
      ...
      for (size_t j = 1; j < new_tree.size() - 1; ++j) {
        if (new_tree[j] == '(' || new_tree[j] == '{') {
          ...
          if (p == 0) {
            left_child = node_unique_ptr(new node_t(this, new_tree.substr(1, j - 1), genome_number, priority_name));
            right_child = node_unique_ptr(new node_t(this, new_tree.substr(j + 1, new_tree.size() - j - 2), genome_number, priority_name));
          }
        }
        if (p < 0) {
