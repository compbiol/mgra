...
template<class graph_pack_t>
void GraphDot<graph_pack_t>::draw_multiedge(std::ofstream &dot, vertex_t const &x, mcolor_t const &color,
                                            vertex_t const &y,
...
void GraphDot<graph_pack_t>::save_subtrees() {
    ...
    //TODO save_subtrees(output, cfg::get().phylotrees);
    output.close();
}
...
void GraphDot<graph_pack_t>::save_subtrees(
    ...
    for (size_t j = 0; j < cfg::get().get_count_genomes(); ++j) {
        output << "\t\"" << cfg::get().priority_name[j] << "\" [fillcolor="
        << cfg::get().get_RGBcolor(j) << "];" << std::endl;
    }

    std::vector<std::string> info;
    for (auto const &tree : trees) {
        std::cerr << cfg::get().mcolor_to_name(tree.get_root()->get_data()) <<
        std::endl;//get_branches_from_tree(tree.get_root(), info); //FIXME
    }
