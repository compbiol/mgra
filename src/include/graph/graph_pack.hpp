...
struct GraphPack {
    ...
    using multicolors_t = graph::graph_pack::Multicolors<mcolor_t>;
    using genome_number_t = typename multicolors_t::genome_number_t;
    using colors_median_t = typename multicolors_t::colors_median_t;

    using history_t = History<mcolor_t>;
    using twobreak_t = typename history_t::twobreak_t;
    using clone_t = typename history_t::clone_t;
    using insdel_t = typename history_t::insdel_t;
    using tandem_duplication_t = typename history_t::tandem_duplication_t;
    ...
    GraphPack(std::vector<MultiGraph::genome_t> const &genomes, tree_t const & tree, genome_number_t const & genome_number, std::string const & target_name)
    : graph(genomes)
    , multicolors(tree, genome_number, target_name)
...
public:
    MultiGraph graph;
    graph::graph_pack::Multicolors<mcolor_t> multicolors;
...
private:
    ...
    std::set<std::tuple<vertex_t, vertex_t, mcolor_t> > canformQ2(vertex_t const &vertex,
                                                                  mcolor_t const &required_color) const;

    void check_changed_vertex(vertex_t const &vertex);

    size_t mobility_score_relative_vertex(vertex_t const &x, mcolor_t const &color, vertex_t const &y,
                                          vertex_t const &about) const;
...
bool GraphPack<mcolor_t>::supercanformQ(vertex_t const &vertex, mcolor_t const &required_color) const {
    ...
    for (auto edge = edges.cbegin(); edge != edges.cend() && !canform; ++edge) {
        ...
        canform = union_color == bar_color_edge;
    }
