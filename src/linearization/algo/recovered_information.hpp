...
struct RecoveredInformation {
    ...
    explicit RecoveredInformation(graph_pack_t const &gp)
            : graph_pack(gp), linearize_algo(gp) {
    ...
    AncestorInformation get_results() const {
        ...
        for (auto const &local_graph : graphs) {
            ret.genomes.push_back(linearize::get_genome(graph_pack, cfg::get().mcolor_to_name(local_graph.first),
                                                        local_graph.second));
        }

        /*for (auto const &elem : transformations) {
            assert(parent_colors.find(elem.first) != parent_colors.cend());
            ret.transformations.insert(
                    std::make_pair(std::make_pair(elem.first, parent_colors.find(elem.first)->second),
                                   elem.second));
        }*/
...
void RecoveredInformation<graph_pack_t>::init_raw_results() {
    for (auto im = graph_pack.multicolors.cbegin_vec_T_consistent_color();
         im != graph_pack.multicolors.cend_vec_T_consistent_color(); ++im) {
...
template<class graph_pack_t>
void RecoveredInformation<graph_pack_t>::init_for_gos_asm() {
    for (size_t i = 0 ; i < cfg::get().get_count_genomes(); ++i) {
        graphs.insert(std::make_pair(mcolor_t(i), graph_pack.graph.get_partgraph(i)));
        transformations.insert(std::make_pair(mcolor_t(i), transform_t()));
    }
}

template<class graph_pack_t>
void RecoveredInformation<graph_pack_t>::walk_and_linearize(
        RecoveredInformation<graph_pack_t>::node_ptr const &current) {
    using namespace linearize;

    if (current->has_left_child()) {
        walk_and_linearize(current->get_left_child());
    }

    if (current->has_right_child()) {
        walk_and_linearize(current->get_right_child());
    }

    if (current->has_left_child() && current->has_right_child() &&
        graphs.find(current->get_data()) != graphs.end()) {
        ...
        if (count_left == 0 && central != 0 && count_right == 0) {
            transform_t linearize_history = linearize_algo.linearize(graphs[current->get_data()],
                                                                     transformations[left->get_data()],
                                                                     graphs[left->get_data()], central);
            ...
            assert(count_circular_chromosome(graph_pack, graphs[current->get_data()]) ==
                   count_circular_chromosome(graph_pack, graphs[left->get_data()]));//Check that all is good
            assert(apply_transformation(graphs[current->get_data()], transformations[left->get_data()]) ==
                   graphs[left->get_data()]); //Check left
            assert(apply_transformation(graphs[current->get_data()], transformations[right->get_data()]) ==
                   graphs[right->get_data()]); //Check right
            if (current->get_parent() != nullptr) {
                auto iter = graphs.find(current->get_parent()->get_data());
                if (iter != graphs.end()) {
                    assert(apply_transformation(iter->second, transformations[current->get_data()]) ==
                           graphs[current->get_data()]); //Check parrent
                }
            }
        }
    }
...
template<class graph_pack_t>
void RecoveredInformation<graph_pack_t>::init_parent_colors(
        RecoveredInformation<graph_pack_t>::node_ptr const &current) {
    if (current->has_left_child()) {
        init_parent_colors(current->get_left_child());
    }

    if (current->has_right_child()) {
        init_parent_colors(current->get_right_child());
    }

    if (current->has_left_child() && current->has_right_child() &&
        graph_pack.multicolors.is_T_consistent_color(current->get_data())) {
        parent_colors.insert(std::make_pair(current->get_left_child()->get_data(), current->get_data()));
        parent_colors.insert(std::make_pair(current->get_right_child()->get_data(), current->get_data()));
    }
}
