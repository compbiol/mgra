...
struct Mcolor {
    ...
    using mcolor_ptr = std::shared_ptr<mcolor_t>;
    using mcolor_ptr_ref = mcolor_ptr &;
    using mcolor_const_ptr = mcolor_ptr const &;
    ...
    : color_size(0)
    {
        ...
        insert(color, multiplicity);
    }

    explicit Mcolor(std::initializer_list<size_t> list)
    : color_size(0)
    {
        for (auto elem : list) {
            insert(elem);
    ...
    void doubling() {
        color_size *= 2;
        for (auto & color : main_color) {
            color.second = color.second * 2;
        }
    }

    void insert(size_t i, size_t multiplicity = 1) {
        assert(multiplicity > 0);
        color_size += multiplicity;
        if (main_color.find(i) == main_color.end()) {
            main_color.insert(std::make_pair(i, multiplicity));
        } else {
            main_color[i] += multiplicity;
        }
    }
    ...
    static Mcolor color_union(color_vector_t const &colors) { //FIXME
        Mcolor result_color;
    ...
    color_vector_t break_into_parts() const { //FIXME
    ...
    size_t how_much_includes(Mcolor const &second) const { //TODO test or think about it
        size_t answer = 0;
        Mcolor current = *this;
    ...
    */
    ...
    size_t size() const { //FIXME later after many test
        size_t size = 0;
        for (auto const &col : main_color) {
            size += col.second;
        }
        assert(size == color_size);
        return color_size;
    }
...
private:
    void update_size() { //remove then modify intersection
       color_size = 0;
        for (auto const &col : main_color) {
            color_size += col.second;
        }
    }
