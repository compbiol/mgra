    ...
    do {
        ...
        for (vertex_t const &v: graph_pack.graph) {
            if (graph_pack.is_simple_vertex(v)) {
                ...
                auto const find_simple_path_lambda = [&](vertex_t const &prev, vertex_t const &cur,
                                                         bool is_next) -> vertex_t {
                    ...
                    while (is_continue) {
                        ...
                        if (processed.find(current) == processed.end()) {
                            ...
                            if ((graph_pack.graph.degree_vertex(current) == 2) &&
                                graph_pack.multicolors.get_complement_color(previous_color) == next_color) {
                                auto const check_lambda = [&](vertex_t const &v) -> bool {
                                    bool flag = true;
                                    std::set<mcolor_t> const &colors = graph_pack.get_all_multicolor_edge_with_info(
                                            current, v);
