        ...
        while (!node_queue.empty()) {
            ...
            if (current_node->is_leaf()) {
                ...
                if (current_node->get_data().size() == 2) {
                    ...
                    current_node->add_children(std::make_shared<node_t>(left));
                    current_node->add_children(std::make_shared<node_t>(right));
                }
            } else {
                node_queue.push(current_node->get_most_left_child());
                node_queue.push(current_node->get_most_right_child());
            }
        }
        return tree;
    }
