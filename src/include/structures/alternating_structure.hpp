...
struct AlternatingStructure {
    ...
    using branch_t = typename graph_pack_t::Statistics::branch_t ;

    AlternatingStructure(vertex_t const & vertex)
            : m_is_cycle(false) {
        m_vertices.push_back(vertex);
    }

    void add_to_right(vertex_t const & vertex) {
        m_vertices.push_back(vertex);
    }

    void add_to_left(vertex_t const & vertex) {
    ...
    std::pair<branch_t, size_t> structure_digest() const {
        return std::make_pair(branch_t(m_colors[0], m_colors[1]), length());
    }
