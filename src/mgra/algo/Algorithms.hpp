...
*/
template<class graph_pack_t>
boost::optional<typename algo::RecoveredInformation<graph_pack_t>::AncestorInformation> main_algorithm(
    ...
    StageManager<graph_pack_t> algorithm(cfg::get().rounds, cfg::get().out_path_to_saves_dir,
    ...
    if (!is_can_reconstruct) {
        INFO("We cannot reconstruct genomes, but we do this for GOS-ASM.")
    }

    INFO("Get results from graphs.")
    RecoveredInformation<graph_pack_t> recover_info(graph_pack);
    recover_info.init_for_gos_asm();
...
template<class graph_pack_t>
boost::optional<typename algo::RecoveredInformation<graph_pack_t>::AncestorInformation> wgd_algorithm(
        graph_pack_t &graph_pack) {
    assert(cfg::get().how_build == default_algo);
