...
bool Algorithm<graph_t>::stage3_1() {
  ...
  for (auto it = graph.begin_vertices(); it != graph.end_vertices(); ++it) { 
    ...
    if (graph.is_indel_vertex(a1) && (processed.find(a1) == processed.end()) && graph.is_indel_vertex(a2) && mul.size() != 0)  {
...
void Algorithm<graph_t>::remove_past_bad_colors() {
  for (auto it = viewed_edges.begin(); it != viewed_edges.end(); ++it) {
    ...
    if (color == graph.get_complete_color()) {
      ...
      //Mcolor bar_color = graph.get_complement_color(it->get_mcolor());
      std::set<Mcolor> colors = graph.split_color(graph.get_complement_color(it->get_mcolor()));
      for (auto im = colors.cbegin(); im != colors.cend(); ++im) {
