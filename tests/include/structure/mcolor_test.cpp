...
TEST_F(McolorTest, SizeFunc) {
    ...
    EXPECT_EQ(colors[0].size(), 0);
    colors[1].doubling();
    EXPECT_EQ(colors[1].size(), 14);

    //Todo here in future
}

TEST_F(McolorTest, DoublingFunc) {
    colors[0].doubling();
    EXPECT_EQ(colors[0], Mcolor());

    Mcolor test(1);
    test.doubling();
    EXPECT_EQ(test, Mcolor({1, 1}));

    test = Mcolor(1, 3);
    test.doubling();
    EXPECT_EQ(test, Mcolor(1, 6));

    colors[5].doubling();
    EXPECT_EQ(colors[5], colors[8]);

    test = colors[10];
    test.doubling();
    EXPECT_EQ(test, Mcolor(colors[10], colors[10], Mcolor::Union));
}
