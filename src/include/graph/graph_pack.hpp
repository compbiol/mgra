...
struct GraphPack {
  ...
  GraphPack(std::vector<MultiGraph::genome_t> const& genomes)
      : graph(genomes), pseudo_infinity_vertex(0), number_of_splits(1), canformQoo(true) {
  }
  ...
  */
  ...
  std::set<mcolor_t> get_all_multicolor_edge_with_info(vertex_t const& u, vertex_t const& v,
                                                       bool with_bad_edge = true) const;
