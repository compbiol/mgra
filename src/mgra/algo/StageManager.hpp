...
struct StageManager {
    struct DebugPolicy {
        ...
        DebugPolicy()
                : is_save_debug(false), path_to_save("") {
        }

        DebugPolicy(bool is_debug, std::string const &save_to)
                : is_save_debug(is_debug), path_to_save(save_to) {
        }
    };

    StageManager(size_t rounds = 1, std::string const &out_path_to_saves_dir = "",
                 DebugPolicy policy = DebugPolicy())
            : debug_policy(policy), number_of_rounds(rounds), path_to_saves_dir(out_path_to_saves_dir) {
